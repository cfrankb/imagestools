...
{
    QString current = m_jsonFile.isEmpty() ? m_imageFile : m_jsonFile;
    if (!current.isEmpty())
    {
        setWindowTitle(QFileInfo(current).fileName() + (m_modified ? " *" : ""));
    }
    else if (m_modified)
    {
        setWindowTitle("*Qt Tile Editor");
...
bool MainWindow::confirmDiscard()
{
    ...
    msg.setText("Discard unsaved changes and load a new file?");
...
void MainWindow::createDockWidget()
{
    ...
    connect(chkUL, &QCheckBox::toggled, this, [this](bool on)
            { onGranularToggle(GranualarUL, on); });
    connect(chkUR, &QCheckBox::toggled, this, [this](bool on)
            { onGranularToggle(GranualarUR, on); });
    connect(chkDL, &QCheckBox::toggled, this, [this](bool on)
            { onGranularToggle(GranualarDL, on); });
    connect(chkDR, &QCheckBox::toggled, this, [this](bool on)
            { onGranularToggle(GranualarDR, on); });
